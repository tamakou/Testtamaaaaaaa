...
public class NetworkRunnerHandler : MonoBehaviour
{
    ...
    protected virtual Task InitializeNetworkRunner(NetworkRunner runner, GameMode gameMode, string sessionName, byte[] connectionToken, NetAddress address, SceneRef scene, Action<NetworkRunner> initialized)
    {
        ...
        customProps["idObject"] = GameManager.instance.idObject;
        customProps["createdIn"] = System.DateTime.Now.ToString("dd/MM/yyyy");
        return runner.StartGame(new StartGameArgs
        {
            ...
            PlayerCount = GameManager.instance.maxPlayer,
    ...
    void HostMigrationResume(NetworkRunner runner)
    {
        ...
        foreach (var resumeNetworkObject in runner.GetResumeSnapshotNetworkObjects())
        {
            ...
            if (resumeNetworkObject.TryGetBehaviour<NetworkWorker>(out var characterController))
            {
                runner.Spawn(resumeNetworkObject, position: null, rotation: null, onBeforeSpawned: (runner, newNetworkObject) =>
                {
                    newNetworkObject.CopyStateFrom(resumeNetworkObject);

                    //Map the connection token with the new Network player
                    if (resumeNetworkObject.TryGetBehaviour<NetworkWorker>(out var oldNetworkPlayer))
                    {
                        // Store Player token for reconnection
                        FindObjectOfType<Spawner>().SetConnectionTokenMapping(oldNetworkPlayer.token, newNetworkObject);
                    }
